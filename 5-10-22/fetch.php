...
$search = str_replace(",", "|", $_POST["query"]);
 $query = "
 SELECT stock.PCode,product.PName,product.PImage,product.QType,product.CID,category.CName,SUM(stock.Qty) AS total_qty,stock.AID,admin.AName,stock.DateAdded,stock.Remarks,stock.Status,product.Stor
...
{
 $query = "SELECT stock.PCode,product.PName,product.PImage,product.QType,product.CID,category.CName,SUM(stock.Qty) AS total_qty,stock.AID,admin.AName,stock.DateAdded,stock.Remarks,stock.Status,product.Stor
									FROM (((stock INNER JOIN product ON stock.PCode = product.PCode)INNER JOIN category ON product.CID = category.CID)INNER JOIN admin ON stock.AID= admin.AID)
